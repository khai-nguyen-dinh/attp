...
class NewsController extends Controller{
	...
	public function do_add_news(){
		//lay du lieu tu bang user, co su dung ham de phan trang
			...
			$c_link_img=url('/public/upload/news').'/'.$c_img;

			//echo $c_link_img;
			$check =DB::table("tbl_news")->where("c_slug","=",$c_slug)->get();

			if($check ==null ){
				DB::insert("insert into tbl_news(c_name,c_description,c_content,c_lang,c_category_news,c_date,c_img,c_slug,c_status) values('$c_name','$c_description','$c_content','$c_lang','$c_category_news','$c_date','$c_link_img','$c_slug',1)");
		...
		//lay du lieu tu bang user, co su dung ham de phan trang
			...
			if($check ==null ){
				...
				if(!empty($c_img)){
					...
					DB::update("UPDATE tbl_news SET c_img='$c_link_img' WHERE pk_news_id=$id");
				}
				return redirect("/admin/news");
			}
			else {
	...
	public function delete_all_news(){
		...
		foreach ($id as $news_id) {
			DB::delete("delete from tbl_news where pk_news_id=$news_id");
		}
		return redirect("/admin/news");
	}
}